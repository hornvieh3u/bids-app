...
function filterby(type, value) {
  for (let child of document.querySelector("tbody").children) {
    if (type === "search") {
      child.style.display =
        child.getHTML().toLowerCase().indexOf(value.toLowerCase()) === -1
          ? "none"
          : "table-row";
    } else if (type === "responsed") {
      child.style.display =
        child.getAttribute("data-is-responsed") != 1 ? "none" : "table-row";
    } else if (type === "timestamp") {
      child.style.display =
        parseInt(child.getAttribute("data-created-at")) < getTodayTimestamp()
          ? "none"
          : "table-row";
    } else {
...
function showChatServers(search) {
  ...
  for (let i = 0; i < chatServerDoms.length; i++) {
    ...
    if (
      chatServerArr.includes(chatServer) ||
      (search && chatServer.indexOf(search) === -1)
    )
      continue;
